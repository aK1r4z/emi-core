...
type (
	...
	SendPrivateMessageRequest struct {
		...
		Message []milky.OutgoingSegment `json:"message"` // 消息内容
	}
	...
	GetMessageRequest struct {
		MessageScene milky.MessageScene `json:"message_scene"` // 消息场景，可能值：friend group temp
		PeerID       int64              `json:"peer_id"`       // 好友 QQ 号或群号
		MessageSeq   int64              `json:"message_seq"`   // 消息序列号
	}

	GetMessageResponse struct {
		Message milky.IncomingMessage `json:"message"` // 消息内容
	}
	...
	GetHistoryMessagesResponse struct {
		Messages       []milky.IncomingMessage `json:"messages"`         // 获取到的消息（message_seq 升序排列），部分消息可能不存在，如撤回的消息
	...
	GetForwardedMessagesResponse struct {
		Messages []milky.IncomingForwardedMessage `json:"messages"` // 转发消息内容
	}
